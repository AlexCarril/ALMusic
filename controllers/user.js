...
module.exports={
    ...
    delete(req,res){
        ...
        function(err,results,fields){
            if(err){
                ...
                res.status(200).send({message: 'registros eliminado'})
                }else{
    ...
    update(req,res){
        ...
        password = data.password;
        console.log(data);
        conexion.query('UPDATE user SET (name,username,password,image) VALUES ("'+name+'","'+username+'","'+password+'")')
    },
   update(req, res) {
        id = req.params.id;
        data = req.body;
            ...
            if(!err){
                if(results.length!=0){
                    ...
                    try{
                        ...
                        conexion.query('UPDATE user SET image = NULL WHERE id = ' + id, function (err, results, fields) {
                            if (!err) {
                                res.status(200).send({ message: "Imagen eliminada y base de datos actualizada" });
                            } else {
                                console.log(err);
                        ...
                        console.log(error)
                        res.status(200).send({message:"No encontrada"})
                    }
